...
# Builds on the official PyTorch image; pre-downloads the SVC checkpoints
# at build time so cold-start workers don't pay the HuggingFace download
# cost on the first job.
...
# Seed-VC sources (this repo is ours; COPY at the end). Install the
# heavy ML deps first so iterating on handler.py doesn't bust the cache.
...
# Pre-fetch the SVC weights so the first job doesn't stall behind a
# huggingface download. Path layout matches Seed-VC's hf_utils helper
...
# Source last so the model layers above stay cached across edits.
COPY . /app
